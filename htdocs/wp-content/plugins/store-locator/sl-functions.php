...
function sl_head_scripts() {
	...
	if (isset($GLOBALS['is_on_sl_page']) && $GLOBALS['is_on_sl_page'] == 1) {
		...
		print "<script src='https://maps.googleapis.com/maps/api/js?v=3{$sens}{$lang_loc}{$region_loc}{$key}' type='text/javascript' id='store-locator'></script>\n";  //v3.96; id='store-locator' prevents removal by sl_remove_gmaps() on sites not observing action hook priorities
...
function sl_remove_gmaps_header_footer($output) {
  if (isset($GLOBALS['is_on_sl_page']) && $GLOBALS['is_on_sl_page'] == 1) {
    if (preg_match("@<script(?!.*id='store-locator')[^>]+maps.google[^>]+>[^<]*<\/script>@", $output)) {
         #ref: Negative lookahead: (?!.*id='store-locator') -- http://stackoverflow.com/a/1240365 - v3.96
    	$output = preg_replace_callback("@<script(?!.*id='store-locator')[^>]+maps.google[^>]+>[^<]*<\/script>@", 
    		create_function('$matches', 'return "<!-- ========= Duplicate Google Maps API JavaScript - Removed by WordPress Store Locator to Avoid Errors. Script HTML:  \n$matches[0] \n========= -->\n";'),
    		$output);
    }
    return $output;
  }
...
function sl_add_options_page() {
   ...
   // if (function_exists("plugins_api")) {
	...
	if (empty($sl_vars['sl_latest_version']) || (time() - strtotime($sl_vars['sl_latest_version_check_time']))/60>=(60*2)){ //2-hr db caching of version info
		/*if (!function_exists("plugins_api")) {
...
function sl_foot_ty($text) {
	...
	if (preg_match("@".SL_DIR."@", $_SERVER['REQUEST_URI']) && (empty($sl_vars['thanks']) || $sl_vars['thanks'] != "true") && mt_rand(1,5) == 3 && defined("SL_THANKS_TIME_LENGTH") && SL_THANKS_TIME_LENGTH != "a few days" && SL_THANKS_NUM_LOCS >= 100) {
