...
/* Requiring CMB2 framework */
require_once dirname( __FILE__ ) . '/inc/class-tgm-plugin-activation.php';


add_action( 'tgmpa_register', 'rtbs_register_required_plugins' );
function rtbs_register_required_plugins() {

	$plugins = array(
		array(
			'name'      => 'CMB2 (plugin tools)',
			'slug'      => 'cmb2',
			'required'  => true,
		),
	);
	...
	$config = array(
		...
		'has_notices'  => true,
		'dismissable'  => false,
		'dismiss_msg'  => '<span style="font-size:15px; font-weight:400;">Responsive Tabs</span>',
		'is_automatic' => true,
		'message'      => '',
	);
...
add_action( 'admin_init', 'add_cmb2_rtbs' );
function add_cmb2_rtbs() {
...
add_action( 'admin_head', 'rtbs_tgm_notice_style' );

function rtbs_tgm_notice_style(){
	if ( !is_plugin_active( WP_PLUGIN_DIR . '/cmb2/init.php' ) ) { ?>
    <style>
			#setting-error-tgmpa {
			  padding-top:3px;
